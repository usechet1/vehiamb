...
async function bulkCreate(inspeccionId, items, empresaId, dbClient = db) {
  ...
  for (const item of items) {
    ...
    const creado = await dbClient.get(
      `INSERT INTO herramientas_items (${FIELDS.join(", ")}) VALUES (${placeholders}) RETURNING *`,
      values
    );
    creados.push(creado);
  }
...
async function findByInspeccion(inspeccionId, empresaId) {
  return db.all(
    "SELECT * FROM herramientas_items WHERE inspeccion_id = ? AND empresa_id = ? ORDER BY id ASC",

...
const ALERTAS = [
  ["Inspección preventiva con hallazgos", "vehicles.edit"],
  ["Vencimiento de documentos", "documents.view"],
  ["Mantenimientos próximos / cambio de aceite", "maintenance.view"],
  ["Comparendos SIMIT", "simit.view"],
  ["Alertas de stock", "inventory.view"],
  ["Actas de entrega/recibido", "delivery.view"],
  ["Gestión de usuarios", "users.manage"]
];

// Debe coincidir con ROLES_SIN_NOTIFICACION_AUTOMATICA en
// services/notificaciones.service.js.
const ROLES_EXCLUIDOS = ["SuperAdministrador"];
...
async function listarDestinatarios() {
  ...
  for (const empresa of empresas) {
    ...
    for (const [descripcion, permiso] of ALERTAS) {
      ...
      finales.forEach((usuario) => console.log(`    - ${formatearUsuario(usuario)}`));
    }
  }
}

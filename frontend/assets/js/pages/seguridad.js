...
function renderExtintores() {
    ...
    if (!filtrados.length) {
        extintoresTablaBody.innerHTML = '<tr><td colspan="5" class="dash-empty">No hay extintores que coincidan con el filtro</td></tr>';
        return;
    }
...
async function cargarExtintores() {
    try {
        ...
        console.error(error);
        extintoresTablaBody.innerHTML = '<tr><td colspan="5" class="dash-empty">No fue posible cargar los extintores</td></tr>';
    }
}
...
function switchTab(tab) {
    const esExtintores = tab === "extintores";

    tabExtintoresButton.classList.toggle("active", esExtintores);
    tabExtintoresButton.setAttribute("aria-selected", String(esExtintores));
    tabBotiquinButton.classList.toggle("active", !esExtintores);
    tabBotiquinButton.setAttribute("aria-selected", String(!esExtintores));

    bloqueExtintores.classList.toggle("hidden", !esExtintores);
    bloqueBotiquin.classList.toggle("hidden", esExtintores);
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        await Promise.all([cargarExtintores(), cargarInspecciones()]);
    } catch (error) {
        console.error(error);

(function () {
    ...
    // ── Reporte PDF: pagina 1 con el consolidado (hora de inspeccion
    // preventiva y preoperacional de cada conductor) y, de la pagina 2 en
    // adelante, una hoja POR CONDUCTOR con el detalle item por item de cada
    // una, claramente dividido. A diferencia de exportReporteImagen (una sola
    // imagen sin paginas) esto si necesita jsPDF real, por eso usa
    // window.VehiAmb.pdfExport.createDocument en vez de Canvas.
    const AZUL_SECCION = [219, 234, 248];
    const VERDE_SECCION = [216, 240, 223];
    const GRIS_TEXTO = [105, 115, 134];
    const NEGRO_TEXTO = [24, 32, 43];
    ...
    const PDF_COLUMNS = [
        { key: "numero", label: "#", width: 25 },
        { key: "conductor", label: "CONDUCTOR", width: 140 },
        { key: "placa", label: "PLACA", width: 55 },
        { key: "ruta", label: "RUTA", width: 230 },
        { key: "inspeccion", label: "HORA INSPECCIÓN", width: 150 },
        { key: "preoperacional", label: "HORA PREOPERACIONAL", width: 162 }
    ];
    ...
    async function addPdfHeader(doc, layout, fecha, totalConductores, branding, titulo) {
        if (branding?.logo) {
            const maxWidth = 90;
            const maxHeight = 47;
            const scale = Math.min(maxWidth / branding.logo.width, maxHeight / branding.logo.height);
        ...
        doc.setFontSize(9);
        doc.setFont(undefined, "bold");
        doc.text("Fecha:", MARGIN_X, layout.y);
        doc.setFont(undefined, "normal");
        doc.text(formatFechaLarga(fecha), MARGIN_X + 40, layout.y);

        doc.setFont(undefined, "bold");
        doc.text("Total de conductores:", MARGIN_X + 300, layout.y);
        doc.setFont(undefined, "normal");
        doc.text(String(totalConductores), MARGIN_X + 430, layout.y);
        layout.y += 22;
    }

    function addPdfTableHeader(doc, layout, columns) {
        doc.setFontSize(8.5);
        doc.setFont(undefined, "bold");
    ...
    // Pagina 1: consolidado uno a uno de conductores, con la hora (no el
    // detalle) de su inspeccion preventiva y preoperacional -- el detalle
    // item por item va en la hoja propia de cada conductor (ver
    // addDetallePorConductor).
    function addConsolidadoTabla(doc, layout, bottomLimit, conductores) {
        const columns = buildPdfColumns();
        addPdfTableHeader(doc, layout, columns);

        conductores.forEach((item, indice) => {
            ...
            const fila = {
                ...
                conductor: safe(item.conductor_nombre, "Sin conductor"),
                placa: safe(item.vehiculo_placa, "Sin vehículo"),
                ruta: safe(item.ruta_nombre),
                inspeccion: formatHora(item.inspeccion?.fecha),
                preoperacional: formatHora(item.preoperacional?.fecha)
            };
            ...
            columns.forEach((column, index) => doc.text(columnLines[index], column.x, layout.y));

            layout.y += rowHeight;
        });
    }

    // Misma mini-tabla (Ítem | Resultado | Observación) para inspeccion
    // preventiva (estado bien/mal) y preoperacional (respuesta si/no) --
    // resaltada en rojo cuando el resultado es el "malo" para que salte a la
    // vista sin tener que leer cada fila.
    function addPdfMiniTabla(doc, layout, bottomLimit, filas) {
        const columnas = [
            { x: MARGIN_X + 10, width: 320 },
            { x: MARGIN_X + 340, width: 80 },
            { x: MARGIN_X + 430, width: layout.pageWidth - MARGIN_X - (MARGIN_X + 430) }
        ];
        ...
        filas.forEach((fila) => {
            ...
            doc.setTextColor(...(fila.mal ? [178, 31, 45] : NEGRO_TEXTO));
    ...
    // Encabezado de seccion en banda de color (una para inspeccion, otra
    // para preoperacional) -- es la "division clara" que se pidio: dos
    // bloques con su propio color e identidad visual, nunca mezclados.
    function addPdfSeccion(doc, layout, bottomLimit, { titulo, color, registro, items, filasFn }) {
        ...
        doc.setFont(undefined, "normal");
        doc.setTextColor(...GRIS_TEXTO);
        doc.text(registro ? `Registrado a las ${formatHora(registro.fecha)}` : "No registrado", layout.pageWidth - MARGIN_X - 10, layout.y, { align: "right" });
        doc.setTextColor(...NEGRO_TEXTO);
        layout.y += 20;
        ...
        if (!registro || !items?.length) {
            ...
            doc.setTextColor(...GRIS_TEXTO);
            doc.text(registro ? "Sin ítems registrados." : `El conductor no ha registrado ${titulo.toLowerCase()}.`, MARGIN_X + 10, layout.y);
            doc.setFont(undefined, "normal");
            doc.setTextColor(...NEGRO_TEXTO);
            layout.y += 18;
            return;
        ...
        addPdfMiniTabla(doc, layout, bottomLimit, filasFn(items));
        layout.y += 12;
    }

    // Pagina 2 en adelante: una hoja por conductor (doc.addPage() SIEMPRE
    // antes de cada uno, a diferencia de mi-viaje-export.js que solo salta
    // de pagina si no cabe -- aca se pidio explicitamente que cada conductor
    // tenga su propia hoja).
    function addDetallePorConductor(doc, layout, bottomLimit, conductores, branding) {
        conductores.forEach((item) => {
            doc.addPage();
            layout.y = 40;

            const vehiculoLabel = `${safe(item.vehiculo_placa, "Sin vehículo")}${item.ruta_nombre ? ` · ${item.ruta_nombre}` : ""}`;
            doc.setFontSize(14);
            doc.setFont(undefined, "bold");
            doc.setTextColor(...NEGRO_TEXTO);
            doc.text(safe(item.conductor_nombre, "Sin conductor"), MARGIN_X, layout.y);
            doc.setFontSize(9.5);
            doc.setFont(undefined, "normal");
            doc.setTextColor(...GRIS_TEXTO);
            doc.text(vehiculoLabel, MARGIN_X, layout.y + 16);
            doc.setTextColor(...NEGRO_TEXTO);
            layout.y += 34;
            doc.setDrawColor(220, 226, 234);
            doc.line(MARGIN_X, layout.y, layout.pageWidth - MARGIN_X, layout.y);
            ...
            addPdfSeccion(doc, layout, bottomLimit, {
                titulo: "Inspección preventiva",
                color: AZUL_SECCION,
                registro: item.inspeccion,
                items: item.inspeccion?.items,
                filasFn: (items) => items.map((detalleItem) => ({
            ...
            addPdfSeccion(doc, layout, bottomLimit, {
                ...
                color: VERDE_SECCION,
                registro: item.preoperacional,
                items: item.preoperacional?.items,
                filasFn: (items) => items.map((detalleItem) => ({
                    item: detalleItem.item_label,
    ...
    async function addPdfFooter(doc, branding) {
    ...
    async function exportReportePdf({ fecha, conductores }) {
        if (!conductores || !conductores.length) {
            throw new Error("No hay asignaciones registradas para esta fecha");
        }
        ...
        const pageWidth = doc.internal.pageSize.getWidth();
        const pageHeight = doc.internal.pageSize.getHeight();
        // El membrete del pie de pagina es un banner muy ancho/bajo (ver
        // getMembreteFooterImage): en horizontal ocupa unos 95pt de alto, asi
        // que se deja el mismo margen que mi-viaje-export.js (130pt) para que
        // el contenido nunca se encime con el.
        ...
        await addPdfHeader(doc, layout, fecha, conductores.length, branding, "REPORTE DE ASIGNACIONES");
        addConsolidadoTabla(doc, layout, bottomLimit, conductores);
        addDetallePorConductor(doc, layout, bottomLimit, conductores, branding);

        await addPdfFooter(doc, branding);

        doc.save(`Reporte_asignaciones_${fecha}.pdf`);
    }

    window.VehiAmb = window.VehiAmb || {};
    window.VehiAmb.asignacionesExport = { exportReporteImagen, exportReporteExcel, exportReportePdf };
})();
